...
import { getAllUrls, getAnalytics, getUserInfo, ShortUrl, AnalyticsData, UserInfo } from '@/lib/api';
...
export function useDashboardData() {
    ...
    const fetchData = async () => {
        try {
            ...
            let analyticsData: { [hash: string]: AnalyticsData } = {};
            if (hashes.length > 0) {
            ...
            const urlsWithAnalytics: DashboardUrl[] = urls.map(url => ({
                ...url,
                analytics: analyticsData[url.shortUrl]
            }));
