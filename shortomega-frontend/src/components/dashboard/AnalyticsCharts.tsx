...
export default function AnalyticsCharts({
  ...
  const topUrlsData = urls
    .filter(
      (url) => url.analytics?.totalVisits && url.analytics.totalVisits > 0
    )
    .sort(
      (a, b) =>
        (b.analytics?.totalVisits || 0) - (a.analytics?.totalVisits || 0)
    )
    .slice(0, 10)
    .map((url) => ({
      name: url.shortUrl,
  ...
  const statusData = [
    ...
    {
      name: "Unused URLs",
      value: urls.filter((url) => (url.analytics?.totalVisits || 0) === 0)
        .length,
      ...
      <Card>
        ...
        >
          <Typography color="text.secondary">
            No analytics data available. Create some short URLs and get clicks
            to see charts!
          </Typography>
        </CardContent>
      </Card>
